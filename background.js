...
async function failNodeFromBackground(nodeId, errorLike = '未知错误') {
  ...
  if (stopRequested || isStopError(errorLike)) {
    const stoppedState = await getState();
    await runBestEffort('complete RegisterExt stopped run', () => completeRegisterManagerRunForState(stoppedState, {
      status: 'stopped',
  ...
  const latestState = await getState();
  try {
    await runBestEffort('complete RegisterExt failed run', () => completeRegisterManagerRunForState(latestState, {
      status: 'failed',
      reason: message,
      failedNodeId: normalizedNodeId,
    }));
...
const plusCheckoutCreateExecutor = self.MultiPageBackgroundPlusCheckoutCreate?.createPlusCheckoutCreateExecutor({
  ...
  getState,
  requestStop,
  registerManagerApiClient: createRegisterManagerApiClientForState(),
